...
import { GetUserLogin, GetCategory, AddCategory,UpdateCategory, DeleteCategory} from '../../../api/user';
...
const Category = () => {
  ...
  const filteredCategory = Categories.filter(category =>
    ...
    (category.categoryCode?.includes(searchTerm) || '')
  );
  
  const totalPages = Math.ceil(filteredCategory.length / recordsPerPage);
  ...
  const currentCategory = filteredCategory.slice(indexOfFirstRecord, indexOfLastRecord);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await AddCategory(updatedFormData);
  ...
  try {
      ...
      if (result.isConfirmed) {
          ...
          if (response.status === 200) {  // Check for successful response
              ...
              setCategory(updatedCategory);  // Update the state with the remaining offices
          } else {
              Swal.fire({
                  title: "Error!",
          ...
          <div className='w-full overflow-x-auto' data-aos='fade-right'>
            <table className='w-full text-sm text-left text-gray-500'>
              ...
              <tr>
                ...
                <th scope="col" className="px-4 py-3 border-t border-r" style={{ minWidth: '100px' }}>Category Code</th>
                <th scope="col" className="px-4 py-3 border-t border-r" style={{ minWidth: '200px' }}>Category Name</th>
              ...
              <tbody>
                {currentCategory.map(category=> (
                  <tr key={category.id} className='transition-colors duration-200 border border-b-gray-200 hover:bg-indigo-50'>
                    ...
                    <td className='px-4 py-4 border-r' >{category.categoryCode}</td>
                    <td className='px-4 py-4 border-r'>{category.categoryName}</td>
    ...
    <div className="relative w-full max-w-2xl mx-auto transition-all transform bg-white shadow-2xl rounded-xl" data-aos='zoom-in'>
      ...
      <div className="px-6 py-6 space-y-6">
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div>
            <label htmlFor="categoryCode" className="block mb-2 text-sm font-semibold text-gray-700">Office Code</label>
            <input
              id="categoryCode"
          ...
          <div>
            <label htmlFor="categoryName" className="block mb-2 text-sm font-semibold text-gray-700">Khmer Name</label>
            <input
              id="categoryName"
              className="block w-full px-4 py-2 text-sm text-gray-800 border border-gray-300 rounded-lg shadow-sm bg-gray-50 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-200"
