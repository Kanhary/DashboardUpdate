...
import { GetUserLogin, GetSubCategory, AddSubCategory, UpdateSubCategory, DeleteSubCategory} from '../../../api/user';
...
const SubCategory = () => {
  ...
  const filteredSubCategory = SubCategories.filter(Subcategory =>
    ...
    Subcategory.subCategoryCode.includes(searchTerm)
  );
  const totalPages = Math.ceil(filteredSubCategory.length / recordsPerPage);
  ...
  const currentSubCategory = filteredSubCategory.slice(indexOfFirstRecord, indexOfLastRecord);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await AddSubCategory(updatedFormData);
  
      // Show success alert
      Swal.fire({
        title: "Saved!",
        text: "Sub-Category has been saved successfully.",
        icon: "success",
  ...
  const handleUpdate = async () => {
    try {
      console.log('Saving Sub-Category data:', formData);
      const id = formData.id;  // Ensure this is valid
      if (!id) {
        Swal.fire({
          title: "Error",
          text: "Sub-Category ID is missing",
          icon: "warning"
        });
        return;
      }
  ...
  try {
      ...
      if (result.isConfirmed) {
          ...
          if (response.status === 200) {  // Check for successful response
              ...
              const updatedSubcategory = SubCategories.filter(Subcategory => Subcategory.id !== id);
              setSubCategory(updatedSubcategory);  // Update the state with the remaining offices
          } else {
              Swal.fire({
                  title: "Error!",
          ...
          <div className='w-full overflow-x-auto' data-aos='fade-right'>
            <table className='w-full text-sm text-left text-gray-500'>
              ...
              <tr>
                ...
                <th scope="col" className="px-4 py-3 border-t border-r" style={{ minWidth: '100px' }}>Sub-Category Code</th>
                <th scope="col" className="px-4 py-3 border-t border-r" style={{ minWidth: '200px' }}>Sub-Category Name</th>
              ...
              <tbody>
                {currentSubCategory.map(Subcategory=> (
                  <tr key={Subcategory.id} className='transition-colors duration-200 border border-b-gray-200 hover:bg-indigo-50'>
                    ...
                    <td className="sticky left-0 h-full px-4 py-3 bg-white border-r" >
                      ...
                      {/* <input type="checkbox" className="mr-1 action-checkbox"/> */}
                        ...
                        <button
                          ...
                          onClick={() => deleteOffice(Subcategory.id)}
                        >
                          <FaTrashAlt />
                        </button>
                      </div>
                    </td>
                    <td className='px-4 py-4 border-r' >{Subcategory.subCategoryCode}</td>
                    <td className='px-4 py-4 border-r'>{Subcategory.subCategoryName}</td>
    ...
    <div className="relative w-full max-w-2xl mx-auto transition-all transform bg-white shadow-2xl rounded-xl" data-aos='zoom-in'>
      ...
      <div className="px-6 py-6 space-y-6">
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div>
            <label htmlFor="subCategoryCode" className="block mb-2 text-sm font-semibold text-gray-700">Office Code</label>
            <input
              id="subCategoryCode"
          ...
          <div>
            <label htmlFor="subCategoryName" className="block mb-2 text-sm font-semibold text-gray-700">Khmer Name</label>
            <input
              id="subCategoryName"
              className="block w-full px-4 py-2 text-sm text-gray-800 border border-gray-300 rounded-lg shadow-sm bg-gray-50 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-200"
