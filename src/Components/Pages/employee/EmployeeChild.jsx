...
import { GetUserLogin, GetAllChild, GetAllStaff, AddChild, UpdateChild, DeleteChild} from '../../../api/user';
...
const EmployeeChild = () => {
  ...
  const [editingChildInfo, setEditingChildInfo] = useState(null);
  ...
  useEffect(() => {
    const fetchAllChild = async () => {
      try {
        const response = await GetAllChild();
  ...
  const filteredChildInfo = ChildInfo.filter(child =>
    (child.EngChildName?.toLowerCase() || '').includes(searchTerm.toLowerCase()) ||
    (child.ChildCode?.includes(searchTerm) || '')
  );
  
  const totalPages = Math.ceil(filteredChildInfo.length / recordsPerPage);
  ...
  const currentChildJob = filteredChildInfo.slice(indexOfFirstRecord, indexOfLastRecord);
  ...
  const closeEditModal = () => {
    setEditingChildInfo(null);
    setFormData({ EngChildName: '', KhChildName: '',ChildCode: '', StaffCode: '', GenderCode: '', BOD: '', ChildeJob: '', Adddres: ''});
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await AddChild(updatedFormData);
  ...
  try {
      ...
      if (result.isConfirmed) {
          ...
          if (response.status === 200) {  // Check for successful response
              ...
              const updatedCategory = ChildInfo.filter(child => child.id !== id);
              setChildInfo(updatedCategory);  // Update the state with the remaining offices
          } else {
              Swal.fire({
                  title: "Error!",
...
const handleDepartmentChange = (selectedOption) => {
  ...
  const handleGenderCode = (option) => {
    console.log('Selected option:', option); // Check if selectedOption has the correct value
    setSelectedOption(option);
    setFormData((prevData) => ({
      ...prevData,
      genderCode: option ? option.value : '',
    }));
  };
  // console.log(Staffs);
          ...
          <div className='w-full overflow-x-auto' data-aos='fade-right'>
            <table className='w-full text-sm text-left text-gray-500'>
              ...
              <tr>
                ...
                <th scope="col" className="px-4 py-3 border-t border-r" style={{ minWidth: '150px' }}>Child Code</th>
                ...
                <th scope="col" className="px-4 py-3 border-t border-r" style={{ minWidth: '150px' }}>Birth Date</th>
                <th scope="col" className="px-4 py-3 border-t border-r" style={{ minWidth: '100px' }}>Child Job</th>
                <th scope="col" className="px-4 py-3 border-t border-r" style={{ minWidth: '100px' }}>Address</th>
              ...
              <tbody>
                {currentChildJob.map(child=> (
                  <tr key={child.id} className='transition-colors duration-200 border border-b-gray-200 hover:bg-indigo-50'>
                    ...
                    <td className='px-4 py-4 border-r' >{child.childCode}</td>
                    <td className='px-4 py-4 border-r'>{child.engChildName}</td>
                    <td className='px-4 py-4 border-r'>{child.khChildName}</td>
                    <td className='px-4 py-4 border-r'>{child.staffCode}</td>
                    <td className='px-4 py-4 border-r'>{child.genderCode}</td>
                    <td className='px-4 py-4 border-r'>{child.bod}</td>
                    <td className='px-4 py-4 border-r'>{child.childJob}</td>
                    <td className='px-4 py-4 border-r'>{child.address}</td>
                    <td className='px-4 py-4 border-r'>{child.LastBy}</td>
                    <td className='px-4 py-4 border-r'>{child.LastDate}</td>
                  </tr>
                ))}
              </tbody>
            </table>
    ...
    <div className="relative w-full max-w-2xl mx-auto transition-all transform bg-white shadow-2xl rounded-xl" data-aos='zoom-in'>
      ...
      <div className="px-6 py-6 space-y-6">
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div>
            <label htmlFor="childCode" className="block mb-2 text-sm font-semibold text-gray-700">Child Code</label>
            <input
              id="childCode"
          ...
          <div>
            <label htmlFor="engChildName" className="block mb-2 text-sm font-semibold text-gray-700">English Name</label>
            <input
              id="engChildName"
              className="block w-full px-4 py-2 text-sm text-gray-800 border border-gray-300 rounded-lg shadow-sm bg-gray-50 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-200"
              value={formData.engChildName}
              onChange={handleChange}
            />
          </div>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          {/* Input for Office Code */}
          <div>
            <label htmlFor="khChildName" className="block mb-2 text-sm font-semibold text-gray-700">Khmer Name</label>
            <input
              id="khChildName"
              className="block w-full px-4 py-2 text-sm text-gray-800 border border-gray-300 rounded-lg shadow-sm bg-gray-50 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-200"
              value={formData.khChildName}
              onChange={handleChange}
            />
          </div>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          {/* Input for Office Code */}
          <div>
          <label htmlFor="genderCode" className="block mb-2 text-sm font-semibold text-gray-700">Gender</label>
            <Select
              options={optionGenderCode}
              onChange={handleGenderCode}  // Ensure handleStaffCode is passed correctly here
              value={optionGenderCode.find(option => option.value === formData.genderCode)}
              placeholder="Select or type to search"
              className="basic-single"
              classNamePrefix="select"
              styles={customStyles}
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          {/* Input for Office Code */}
          <div>
            <label htmlFor="childJob" className="block mb-2 text-sm font-semibold text-gray-700">Child Job</label>
            <input
              id="childJob"
              className="block w-full px-4 py-2 text-sm text-gray-800 border border-gray-300 rounded-lg shadow-sm bg-gray-50 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-200"
